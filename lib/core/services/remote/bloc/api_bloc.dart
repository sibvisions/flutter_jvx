...
class ApiBloc extends Bloc<Request, Response> {
  ...
  void onEvent(Request event) {
    if (event.requestType != RequestType.RELOAD) {
      event.id = _seqNo++;
      print(
          '******* Outgoing RequestID: ${event.id}, Type: ${event.requestType.toString().replaceAll("RequestType.", "")}, Info: ${event.debugInfo}');
    }
    ...
    if (await this.networkInfo.isConnected) {
      await for (Response response
        ...
        if (response.request.requestType != RequestType.LOADING &&
          ...
          print(
              '******* Incoming RequestID: ${response.request.id}, Type: ${response.request.requestType.toString().replaceAll("RequestType.", "")}, Info: ${response.request.debugInfo}');
        }
