...
class JVxFormLayoutConstraintData extends ParentDataWidget<JVxFormLayoutWidget> {
  ...
  bool debugIsValidAncestor(RenderObjectWidget ancestor) {
    assert(JVxFormLayoutWidget != dynamic);
    assert(JVxFormLayoutWidget != RenderObjectWidget);
    return ancestor is JVxFormLayoutWidget;
  }
  ...
  Iterable<DiagnosticsNode> debugDescribeInvalidAncestorChain({ String description, DiagnosticsNode ownershipChain, bool foundValidAncestor, Iterable<Widget> badAncestors }) sync* {
    ...
    if (!foundValidAncestor) {
      yield ErrorDescription(
        '$runtimeType widgets must be placed inside $JVxFormLayoutWidget widgets.\n'
        '$description has no $JVxFormLayoutWidget ancestor at all.'
      );
    } else {
      assert(badAncestors.isNotEmpty);
      yield ErrorDescription(
        '$runtimeType widgets must be placed directly inside $JVxFormLayoutWidget widgets.\n'
        '$description has a $JVxFormLayoutWidget ancestor, but there are other widgets between them:'
      );
      ...
      yield ErrorDescription('These widgets cannot come between a $runtimeType and its $JVxFormLayoutWidget.');
    }
