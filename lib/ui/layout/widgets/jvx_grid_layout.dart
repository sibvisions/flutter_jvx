...
///
class JVxGridLayoutWidget extends MultiChildRenderObjectWidget {
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	// Class members
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  JVxGridLayoutWidget({
    Key key,
    List<JVxGridLayoutConstraintData> children: const [],
    this.rows = 1,
    this.columns = 1,
    this.margins = EdgeInsets.zero,
    this.horizontalGap = 0,
    this.verticalGap = 0 }) : super (key: key, children: children);

  @override
  RenderObject createRenderObject(BuildContext context) {
    return RenderJVxGridLayoutWidget(this.rows, this.columns, this.margins, this.horizontalGap, this.verticalGap);
  }

  @override
  void updateRenderObject(BuildContext context, RenderJVxGridLayoutWidget renderObject) {
...
class RenderJVxGridLayoutWidget extends RenderBox
    with ContainerRenderObjectMixin<RenderBox, MultiChildLayoutParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, MultiChildLayoutParentData> {

  // Stores all constraints.
	Map<RenderBox, GridLayoutConstraints>	constraintMap = <RenderBox, GridLayoutConstraints>{};
  ...
  void performLayout() {
    // Set components
    constraintMap = <RenderBox, GridLayoutConstraints>{}; 
    ...
    if (targetColumns > 0 && targetRows > 0)
    {
      ...
      int totalWidth = size.width.round() - leftInsets - margins.right.round() - totalGapsWidth;
      int totalHeight = size.height.round() - topInsets - margins.bottom.round() - totalGapsHeight;
      ...
      constraintMap?.forEach((component, constraints) {
          ...
          double x = getPosition(xPosition, constraints.gridX, columnSize, horizontalGap) + insets.left;
          double y = getPosition(yPosition, constraints.gridY, rowSize, verticalGap) + insets.top;
          double width = getPosition(xPosition, constraints.gridX + constraints.gridWidth, columnSize, horizontalGap) - horizontalGap - x - insets.right;
          double height = getPosition(yPosition, constraints.gridY + constraints.gridHeight, rowSize, verticalGap) - verticalGap - y - insets.bottom;

          component.layout(BoxConstraints(minWidth: width, maxWidth: width, minHeight: height, maxHeight: height), parentUsesSize: true);
  ...
  /*
	 ...
	 * @return the position
	 */
	static int getPosition(List<int> pPositions, int pIndex, int pSize, int pGap)
	{
		if (pIndex < 0)
		{
  ...
  void addLayoutComponent(RenderBox pComp, Object pConstraints)
    ...
    if (pConstraints is GridLayoutConstraints)
		{
  ...
  /*
	 ...
	 */
	void setConstraints(RenderBox pComponent, GridLayoutConstraints pConstraints)
	{
...
class JVxGridLayoutConstraintData extends ParentDataWidget<MultiChildLayoutParentData> {
  ...
  final GridLayoutConstraints id;
