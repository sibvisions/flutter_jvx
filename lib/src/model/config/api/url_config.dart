class UrlConfig {
  ...
  UrlConfig({required this.host, required this.path, required this.https, this.port});
  ...
  static int? extractPort({required String url}) {
    ...
    if (containsHttp) {
      ...
      if (containsPort) {
        int indexOfStart = split.indexOf(":") + 1;
        int portLength = split.indexOf("/", split.indexOf(":")) - indexOfStart;
        return int.parse(split.substring(indexOfStart, indexOfStart + portLength));
      }
    }
    return null;
  }
