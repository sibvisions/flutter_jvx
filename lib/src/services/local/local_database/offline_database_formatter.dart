...
class OfflineDatabaseFormatter {
  ...
  static String _getWhereCondition(
      String? columnName, dynamic? value, CompareType? compareType,
      [bool? not = false]) {
    if (value != null) {
      String comparer = "=";
      if (compareType == CompareType.EQUALS)
        ...
        comparer = '>';
      else if (compareType == CompareType.GREATER_EQUALS) comparer = '>=';
      if (not != null && not)
  ...
  static String getWhereFilter(List<dynamic>? columnNames,
      List<dynamic>? values, List<FilterCompareOperator>? compareOperators) {
    String sqlWhere = "";
    if (columnNames != null &&
        values != null &&
  ...
  static String getDataType(CellEditor editor) {
    switch (editor.className) {
      ...
      case 'NumberCellEditor':
        if (editor.getProperty<int>(CellEditorProperty.SCALE, null) == 0)
          return "INTEGER";
        else
          return "NUMERIC";
      case 'DateCellEditor':
